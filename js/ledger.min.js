        ...
        9: [function(require, module, exports) {
            (function(Buffer) {
                ...
                LedgerBtc.prototype.getWalletPublicKey_async = function(path) {
                    ...
                    var self = this;
                    return this.comm.exchange(buffer.toString("hex"), [36864]).then(function(response) {
                        var result = {};
                ...
                LedgerBtc.prototype.getTrustedInputRaw_async = function(firstRound, indexLookup, transactionData) {
                    ...
                    buffer[0] = 224;
                    buffer[1] = 66;
                    buffer[2] = firstRound ? 0 : 128;
                    buffer[3] = 0;
                    buffer[4] = data.length;
                    buffer = Buffer.concat([buffer, data], 5 + data.length);
                    return this.comm.exchange(buffer.toString("hex"), [36864]).then(function(trustedInput) {
                ...
                LedgerBtc.prototype.getTrustedInput_async = function(indexLookup, transaction) {
                    ...
                    var processScriptBlocks = function(script, sequence) {
                        ...
                        while (offset != script.length) {
                            var blockSize = script.length - offset > LedgerBtc.MAX_SCRIPT_BLOCK ? LedgerBtc.MAX_SCRIPT_BLOCK : script.length - offset;
                        ...
                        return utils.eachSeries(scriptBlocks, function(scriptBlock) {
                            return currentObject.getTrustedInputRaw_async(false, undefined, scriptBlock)
                        })
                    };
                    var processInputs = function() {
                        return utils.eachSeries(transaction["inputs"], function(input) {
                    ...
                    var processOutputs = function() {
                        return utils.eachSeries(transaction["outputs"], function(output) {
                            data = output["amount"];
                    ...
                    return currentObject.getTrustedInputRaw_async(true, indexLookup, data).then(processInputs).then(processOutputs)
                }
                ;
                            ...
                            return currentObject.startUntrustedHashTransactionInputRaw_async(newTransaction, false, data).then(function(result) {
                                ...
                                if (input["script"].length == 0) {
                                    ...
                                    while (offset != input["script"].length) {
                                        var blockSize = input["script"].length - offset > LedgerBtc.MAX_SCRIPT_BLOCK ? LedgerBtc.MAX_SCRIPT_BLOCK : input["script"].length - offset;
                ...
                LedgerBtc.prototype.hashOutputFull_async = function(outputScript) {
                    var offset = 0;
                    var self = this;
                    return utils.asyncWhile(function() {
                        ...
                        var p1 = offset + blockSize == outputScript.length ? 128 : 0;
                        ...
                        return self.comm.exchange(data.toString("hex"), [36864]).then(function(data) {
                            offset += blockSize
                        })
                    })
                }
                ...
                LedgerBtc.prototype.signTransaction_async = function(path, lockTime, sigHashType) {
                    ...
                    var self = this;
                    return self.comm.exchange(buffer.toString("hex"), [36864]).then(function(signature) {
                ...
                LedgerBtc.prototype.signMessageNew_async = function(path, messageHex) {
                    ...
                    return utils.foreach(apdus, function(apdu) {
                        return self.comm.exchange(apdu, [36864]).then(function(apduResponse) {
                        ...
                        buffer[4] = 1;
                        buffer[5] = 0;
                        return self.comm.exchange(buffer.toString("hex"), [36864]).then(function(apduResponse) {
                            var response = Buffer.from(apduResponse, "hex");
                            var result = {};
                            result["v"] = response[0] - 48;
                ...
                LedgerBtc.prototype.createPaymentTransactionNew_async = function(inputs, associatedKeysets, changePath, outputScript, lockTime, sigHashType) {
                    ...
                    utils.foreach(inputs, function(input, i) {
                        return utils.doIf(!resuming, function() {
                            return self.getTrustedInput_async(input[1], input[0]).then(function(trustedInput) {
                                var inputItem = {};
                        ...
                        for (var i = 0; i < inputs.length; i++) {
                            ...
                            targetTransaction["inputs"][i]["prevout"] = trustedInputs[i]["value"].slice(4, 4 + 36)
                        }
                        var lockTimeBuffer = Buffer.alloc(4);
                        lockTimeBuffer.writeUInt32LE(lockTime, 0);
                        var result = Buffer.concat([self.serializeTransaction(targetTransaction), outputScript, lockTimeBuffer]);
                    ...
                    utils.foreach(inputs, function(input, i) {
                        return utils.doIf(!resuming, function() {
                            return self.getTrustedInput_async(input[1], input[0]).then(function(trustedInput) {
                                ...
                                inputItem["value"] = Buffer.from(trustedInput, "hex").slice(4, 4 + 36);
                                trustedInputs.push(inputItem)
                            })
                        }).then(function() {
                            regularOutputs.push(input[0].outputs[input[1]])
                ...
                LedgerBtc.prototype.compressPublicKey = function(publicKey) {
                    var prefix = (publicKey[64] & 1) != 0 ? 3 : 2;
                ...
                LedgerBtc.prototype.serializeTransactionOutputs = function(transaction) {
                    ...
                    if (typeof transaction["outputs"] != "undefined") {
                        ...
                        transaction["outputs"].forEach(function(output) {
                            outputBuffer = Buffer.concat([outputBuffer, output["amount"], self.createVarint(output["script"].length), output["script"]])
                        })
                    }
                    return outputBuffer
                }
                ...
                LedgerBtc.prototype.serializeTransaction = function(transaction) {
                    ...
                    transaction["inputs"].forEach(function(input) {
                        inputBuffer = Buffer.concat([inputBuffer, input["prevout"], self.createVarint(input["script"].length), input["script"], input["sequence"]])
                    });
                    ...
                    return Buffer.concat([transaction["version"], self.createVarint(transaction["inputs"].length), inputBuffer, outputBuffer])
                }
                ;
                ...
                LedgerBtc.DEFAULT_SEQUENCE = 4294967295;
        ...
        11: [function(require, module, exports) {
            (function(Buffer) {
                ...
                LedgerEth.prototype.getAddress_async = function(path, boolDisplay, boolChaincode) {
                    ...
                    buffer[0] = 224;
                    buffer[1] = 2;
                    buffer[2] = boolDisplay ? 1 : 0;
                    buffer[3] = boolChaincode ? 1 : 0;
                    buffer[4] = 1 + splitPath.length * 4;
                    buffer[5] = splitPath.length;
                    splitPath.forEach(function(element, index) {
                        buffer.writeUInt32BE(element, 6 + 4 * index)
